  ...
  useEffect(() => {
    fetch('https://opendata.zoneatlas.com/oulu/objects.json')
      ...
      .then((json) => {
        ...
        setTotalPages(Math.ceil(fireplaceObjects.length / itemsPerPage));
      })
  ...
  const loadMoreData = () => {
    ...
    if (currentPage < totalPages) {
      ...
      fetch(`https://opendata.zoneatlas.com/oulu/objects.json?start=${start}&end=${end}`)
        .then((response) => response.json())
        .then((json) => {
          const fireplaceObjects = json.filter((item) => {
            const Fireplaces = item.Categories.find((category) => category.title === 'Tulentekopaikka');
            return !!Fireplaces;
          });
          setData([...data, ...fireplaceObjects]);
        })
        .catch((error) => console.error(error));
    }
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      <FlatList
        ...
        data={data}
        renderItem={({ item }) => {
          const Fireplaces = item.Categories.find(
            (category) => category.title === 'Tulentekopaikka'
          );
          if (!Fireplaces) {
            return null;
          }
          ...
          return (
            <View key={item.id} style={[{
              ...
              <Modal visible={showModal} animationType="none">
                ...
                {selectedObject && (
                  <Details2 object={selectedObject} mode={mode} PlaceholderImage={PlaceholderImage}/>
                )}
